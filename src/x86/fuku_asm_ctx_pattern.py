from x86.fuku_operand import FukuPrefix
...
class FukuAsmCtxPattern:
    ...
    def gen_pattern64_1em_immb(self, byte1, rm_reg, imm):
        self.emit_rex_64(rm_reg)
        self.emit_b(byte1)
        self.emit_immediate_b(imm)
    ...
    def gen_pattern64_1em_immw(self, byte1, rm_reg, imm):
        self.emit_rex_64(rm_reg)
        self.emit_b(byte1)
        self.emit_immediate_w(imm)
    ...
    def gen_pattern32_2em_immb(self, byte1, byte2, rm_reg, imm):
        self.emit_optional_rex_32(rm_reg)
        self.emit_b(byte1)
        self.emit_b(byte2)
        self.emit_immediate_b(imm)

    def gen_pattern64_2em_immb(self, byte1, byte2, rm_reg, imm):
        self.emit_rex_64(rm_reg)
        self.emit_b(byte1)
        self.emit_b(byte2)
        self.emit_immediate_b(imm)
    ...
    def gen_pattern32_2em_immw(self, byte1, byte2, rm_reg, imm):
        self.emit_optional_rex_32(rm_reg)
        self.emit_b(byte1)
        self.emit_b(byte2)
        self.emit_immediate_w(imm)

    def gen_pattern64_2em_immw(self, byte1, byte2, rm_reg, imm):
        self.emit_rex_64(rm_reg)
        self.emit_b(byte1)
        self.emit_b(byte2)
        self.emit_immediate_w(imm)
    ...
    def gen_pattern64_2em_immdw(self, byte1, byte2, rm_reg, imm):
        self.emit_rex_64(rm_reg)
        self.emit_b(byte1)
        self.emit_b(byte2)
        self.emit_immediate_dw(imm)
    ...
    def gen_pattern64_1em_op_idx_immw(self, byte1, operand, idx, imm):
        self.gen_pattern64_1em_op_idx(byte1, operand, idx)
        self.emit_immediate_w(imm)
    ...
    def gen_pattern32_2em_op_idx_immw(self, byte1, byte2, operand, idx, imm):
        self.gen_pattern32_2em_op_idx(byte1, byte2, operand, idx)
        self.emit_immediate_w(imm)

    def gen_pattern64_2em_op_idx_immw(self, byte1, byte2, operand, idx, imm):
        self.gen_pattern64_2em_op_idx(byte1, byte2, operand, idx)
        self.emit_immediate_w(imm)

    # emit 2 bytes , emit operand(regrm, idx) imm dw
    def gen_pattern32_2em_op_idx_immdw(self, byte1, byte2, operand, idx, imm):
        self.gen_pattern32_2em_op_idx(byte1, byte2, operand, idx)
        self.emit_immediate_dw(imm)

    def gen_pattern64_2em_op_idx_immdw(self, byte1, operand, idx, imm):
        self.gen_pattern64_2em_op_idx(byte1, operand, idx)
        self.emit_immediate_dw(imm)
    ...
    def gen_pattern64_1em_rm_idx_immw(self, byte1, rm_reg, idx, imm):
        self.gen_pattern64_1em_rm_idx(byte1, rm_reg, idx)
        self.emit_immediate_w(imm)
    ...
    def gen_pattern32_2em_rm_idx_immw(self, byte1, byte2, rm_reg, idx, imm):
        self.gen_pattern32_2em_rm_idx(byte1, byte2, rm_reg, idx)
        self.emit_immediate_w(imm)

    def gen_pattern64_2em_rm_idx_immw(self, byte1, byte2, rm_reg, idx, imm):
        self.gen_pattern64_2em_rm_idx(byte1, byte2, rm_reg, idx)
        self.emit_immediate_w(imm)

    # emit 2 bytes , emit rm(regrm, idx) imm dw
    def gen_pattern32_2em_rm_idx_immdw(self, byte1, byte2, rm_reg, idx, imm):
        self.gen_pattern32_2em_rm_idx(byte1, byte2, rm_reg, idx)
        self.emit_immediate_dw(imm)

    def gen_pattern64_2em_rm_idx_immdw(self, byte1, rm_reg, idx, imm):
        self.gen_pattern64_2em_rm_idx(byte1, rm_reg, idx)
        self.emit_immediate_dw(imm)
