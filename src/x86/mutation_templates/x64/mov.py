...
def _mov_64_multi_tmpl_3(ctx: FukuMutationCtx, dst: FukuType, src: FukuType, inst_size: int) -> bool:
    ...
    if (
         (dst.register.index == FukuRegisterIndex.INDEX_SP if dst.type == FukuT0Types.FUKU_T0_REGISTER else False) or
         (src.register.index == FukuRegisterIndex.INDEX_SP if src.type == FukuT0Types.FUKU_T0_REGISTER else False)
    ):
        return False
...
def _mov_64_reg_imm_tmpl_1(ctx, dst: FukuType, src: FukuType, inst_size: int) -> bool:
    ...
    trace_inst("mov dst, src -> xor dst, dst; add dst, imm_0; add dst, imm_1; ...", opcodes)
    return True
...
def _mov_64_reg_imm_tmpl(ctx: FukuMutationCtx) -> bool:
    ...
    match rng.randint(0, 3):
        ...
        case 3:
            return _mov_64_reg_imm_tmpl_1(ctx, reg_dst, imm_src, ctx.instruction.operands[0].size)
