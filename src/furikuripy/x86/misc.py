...
from furikuripy.x86.fuku_register_math_tables import CAPSTONE_JCC, CAPSTONE_SETCC, CAPSTONE_MOVCC
...
class FukuCondition(Enum):
    NO_CONDITION = -1;                jmp = -1

    OVERFLOW        = 0;    jo   = 0;             # (OF == 1)
    NO_OVERFLOW     = 1;    jno  = 1;             # (OF != 1)
    BELOW           = 2;    jb   = 2;             # (CF == 1)
    ABOVE_EQUAL     = 3;    jae  = 3;    jnc = 3  # (CF != 1)
    EQUAL           = 4;    je   = 4;    jz  = 4  # (ZF == 1)
    NOT_EQUAL       = 5;    jne  = 5;    jnz = 5  # (ZF != 1)
    BELOW_EQUAL     = 6;    jbe  = 6;    jna = 6  # (CF == 1 || ZF == 1)
    ABOVE           = 7;    jnbe = 7;    ja  = 7  # (CF != 1 && ZF != 1)
    NEGATIVE        = 8;    js   = 8;             # (SF == 1)
    POSITIVE        = 9;    jns  = 9;             # (SF != 1)
    PARITY_EVEN     = 10;   jp   = 10;            # (PF == 1)
    PARITY_ODD      = 11;   jnp  = 11;   jpo = 11 # (PF != 1)
    LESS            = 12;   jnge = 12;   jl  = 12 # (SF != OF)
    GREATER_EQUAL   = 13;   jge  = 13;   jnl = 13 # (SF == OF)
    LESS_EQUAL      = 14;   jng  = 14;   jle = 14 # (ZF == 1 || (SF != OF) )
    GREATER         = 15;   jnle = 15;   jg  = 15 # (ZF != 1 && (SF == OF) )
