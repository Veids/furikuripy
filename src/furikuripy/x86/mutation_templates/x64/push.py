...
def _push_64_multi_tmpl_1(ctx: FukuMutationCtx, src: FukuType, inst_size: int) -> bool:
    if (
        ...
        and src.register.index == FukuRegisterIndex.INDEX_SP
    ):
        return False
    ...
    trace_inst(
        "push src -> sub rsp, 8 or lea rsp, [rsp, 8]; mov [rsp], reg", opcodes, ctx
    )
    return True
...
def _push_64_multi_tmpl_2(ctx: FukuMutationCtx, src: FukuType, inst_size: int) -> bool:
    ...
    trace_inst(
        "push src -> mov [rsp - 8], reg; sub rsp, 8 or lea rsp, [rsp, 8]", opcodes, ctx
    )
    return True
