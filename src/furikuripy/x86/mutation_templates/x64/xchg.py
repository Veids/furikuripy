...
# xor dst_2, dst_1
# xor dst_1, dst_2
def _xchg_64_multi_tmpl_1(
    ctx: FukuMutationCtx, dst_1: FukuType, dst_2: FukuType, inst_size: int
) -> bool:
    ...
    changes_regflags = ctx.cpu_registers & ~(
        dst_1.get_mask_register() | dst_2.get_mask_register()
    )
    ...
    trace_inst(
        "xchg dst_1, dst_2 -> xor dst_1, dst_2; xor dst_2, dst_1, xor dst_1, dst_2",
        opcodes,
        ctx,
    )
    return True
...
def _xchg_64_multi_tmpl_2(
    ...
    if dst_1.type == FukuT0Types.FUKU_T0_OPERAND:
        if (
            dst_1.operand.base.index == FukuRegisterIndex.INDEX_SP
            or dst_1.operand.index.index == FukuRegisterIndex.INDEX_SP
        ):
    ...
    trace_inst(
        "xchg dst_1, dst_2 -> mov reg_1, dst_1; mov reg_2, dst_2, mov dst_1, reg_2; mov dst_2, reg_1",
        opcodes,
        ctx,
    )
    return True
...
def _xchg_64_reg_reg_tmpl(ctx: FukuMutationCtx) -> bool:
    ...
    match rng.randint(0, 1):
        case 0:
            return _xchg_64_multi_tmpl_1(
                ctx, reg_dst_1, reg_dst_2, ctx.instruction.operands[0].size
            )

        case 1:
            return _xchg_64_multi_tmpl_2(
                ctx, reg_dst_1, reg_dst_2, ctx.instruction.operands[0].size
            )
...
def _xchg_64_op_reg_tmpl(ctx: FukuMutationCtx) -> bool:
    ...
    return _xchg_64_multi_tmpl_2(
        ctx, op_dst.ftype, reg_dst.ftype, ctx.instruction.operand[0].size
    )


def fukutate_64_xchg(ctx: FukuMutationCtx) -> bool:
    if (
        ctx.instruction.operands[0].type == x86_const.X86_OP_MEM
        or ctx.instruction.operands[1].type == x86_const.X86_OP_MEM
    ):
